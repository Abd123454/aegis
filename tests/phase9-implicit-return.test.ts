...
describe("Phase 9: Implicit return bypass — fs (R5-LIE-fs-implicit)", () => {
  ...
  test("R5-LIE-fs-implicit: declare -> MyS, implicit return env.fs", async () => {
...
describe("Phase 9: Implicit return bypass — db (R5-sql-implicit)", () => {
  test("R5-sql-implicit: declare -> MyS, implicit return env.db", async () => {
    const r = await run(`struct MyS { x: Int }
    impl MyS {
...
describe("Phase 9: Implicit return bypass — shell (R5-cmd-implicit)", () => {
  test("R5-cmd-implicit: declare -> MyS, implicit return env.shell", async () => {
    const r = await run(`struct MyS { x: Int }
    impl MyS {
...
describe("Phase 9: Struct literal depth (NEST-struct)", () => {
  ...
  test("NEST-struct: 300 nested struct literals produce clean error", async () => {
  ...
  test("NEST-struct-ok: 100 nested struct literals work", async () => {
...
describe("Phase 9: Implicit return — legitimate cases still work", () => {
  ...
  test("IMPL-RET-OK: implicit return of correct type works", async () => {
    const r = await run(`fn f(x: Int) -> Int { x + 1 }
    fn main() { print(f(5)) }`);
  ...
  test("IMPL-RET-IF: implicit return via if-expression works", async () => {
...
describe("Phase 9: Explicit return still checked (regression)", () => {
  ...
  test("EXPL-RET-LIE: explicit return of wrong type rejected", async () => {
    const r = await run(`struct MyS { x: Int }
