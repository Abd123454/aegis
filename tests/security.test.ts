...
describe("Security: rejected by construction", () => {
  test("null dereference — `null` is not a token", async () => {
    const r = await run(`fn main() { let x = null }`);
    expect(r.ok).toBe(false);
    expect(r.diagnostics.some((d) => /no `null`/.test(d.msg))).toBe(true);
  });

  test("use-after-free — `malloc`/`free` do not exist", async () => {
    const r = await run(`fn main() { let p = malloc(4); free(p); print(p) }`);
    expect(r.ok).toBe(false);
    expect(r.diagnostics.some((d) => /malloc|free/.test(d.msg))).toBe(true);
  });

  test("double-free — same mechanism (no free at all)", async () => {
    const r = await run(`fn main() { free("x") }`);
    expect(r.ok).toBe(false);
  });

  test("SQL injection — db.query(string) rejected at analysis", async () => {
  ...
  test("command injection — shell.run(string) rejected at analysis", async () => {
    const r = await run(`fn main(env: Cap) {
  ...
  test("data race — `static mut` is forbidden", async () => {
    const r = await run(`static mut counter = 0\nfn main() { spawn(move || { }) }`);
    expect(r.ok).toBe(false);
    expect(r.diagnostics.some((d) => /static mut/.test(d.msg))).toBe(true);
  });

  test("ambient authority — fs.read without Cap rejected at analysis", async () => {
    const r = await run(`fn process(input: String) { fs.read(input) }
    fn main(env: Cap) { process("/etc/passwd") }`);
    expect(r.ok).toBe(false);
    expect(r.diagnostics.some((d) => /capability|Cap/.test(d.msg))).toBe(true);
  });

  test("buffer overflow — out-of-bounds index returns None, not UB", async () => {
  ...
  test("integer overflow — checked arithmetic returns Err", async () => {
    const r = await run(`fn main() {
      let big = 2000000000
      let sum = big + big
      match sum {
  ...
  test("raw pointer — `&` is not in the safe subset", async () => {
    const r = await run(`fn main() { let x = &5 }`);
    expect(r.ok).toBe(false);
    expect(r.diagnostics.some((d) => /Raw references|&/.test(d.msg))).toBe(true);
  });

  test("spawn without `move` is rejected", async () => {
    const r = await run(`fn main() { spawn(|| { }) }`);
    expect(r.ok).toBe(false);
    expect(r.diagnostics.some((d) => /move/.test(d.msg))).toBe(true);
  });

  test("non-exhaustive match is a runtime error (not silent fallthrough)", async () => {
  ...
  test("calling a non-function fails cleanly", async () => {
    const r = await run(`fn main() { let x = 5; x() }`);
    expect(r.ok).toBe(false);
  });
});

describe("Security: brevity features do NOT weaken guarantees", () => {
  test("SQL injection rejected even with brevity syntax", async () => {
    const r = await run(`fn main(env: Cap) {
      let u = "admin"
  ...
  test("command injection rejected even with brevity syntax", async () => {
    const r = await run(`fn main(env: Cap) { env.shell.run("ls " + "x") }`);
    expect(r.ok).toBe(false);
  });

  test("static mut rejected even in one line", async () => {
    const r = await run(`static mut c = 0`);
    expect(r.ok).toBe(false);
  });

  test("ambient fs.read rejected even via lambda + pipeline", async () => {
    const r = await run(`fn go(f) { f("/etc/passwd") }
