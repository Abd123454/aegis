...
describe("Phase 13: async/await syntax", () => {
  test("ASYNC-1: async fn parses and runs", () => {
    const r = run(`async fn greet(name: String) -> String {
      "Hello, " + name
    }
    fn main() {
      let r = greet("World")
  ...
  test("ASYNC-2: await expression works (treated as identity in sync evaluator)", () => {
    const r = run(`fn main() {
      let x = await 42
      print(x)
    }`);
  ...
  test("ASYNC-3: async fn with await and Cap", () => {
    const r = run(`async fn get_data(env: Cap) -> String {
...
describe("Phase 13: Real AI (mock mode for tests)", () => {
  test("AI-REAL-1: complete returns response", () => {
  ...
  test("AI-REAL-2: await with AI call", () => {
    const r = run(`async fn ask(env: Cap) -> String {
  ...
  test("AI-REAL-3: AI capability still gated", () => {
    const r = run(`fn sneaky() { env.ai.complete("test") }
    fn main() { sneaky() }`);
...
describe("Phase 13: Structured diagnostics", () => {
  test("DIAG-1: type error includes function name", () => {
  ...
  test("DIAG-2: capability error names the method", () => {
    const r = run(`fn main() { env.fs.read("x") }`);
    expect(r.ok).toBe(false);
...
describe("Phase 13: CLI --json flag", () => {
  test("JSON-FLAG: cli.ts exists and has --json handling", () => {
    const fs = require("fs");
...
describe("Phase 13: stdlib completeness", () => {
  test("STDLIB-1: all fs methods available", () => {
    const r = run(`fn main(env: Cap) {
      env.fs.write("a", "b")?
      env.fs.read("a")?
      env.fs.list(".")?
  ...
  test("STDLIB-3: crypto functions", () => {
    const r = run(`fn main() {
      let h = sha256("test")
      let r = random_hex(8)
      print("crypto ok")
