...
describe("Phase 10: Option unwrapping (F1, F2)", () => {
  ...
  test("BYPASS-CHECK: (Some(env.fs))? as implicit return to MyS — rejected", async () => {
  ...
  test("ARRAY-CAP: array of Cap indexed then .read() works", async () => {
    const r = await run(`fn main(env: Cap) {
        let arr = [env.fs]
        let fs = arr[0]?
        fs.read("test")?
  ...
  test("MAP-CAP: map with Cap value, get then .read() works", async () => {
    const r = await run(`fn main(env: Cap) {
        let m = #{ "fs": env.fs }
        let fs = m.get("fs")?
        fs.read("test")?
...
describe("Phase 10: Test isolation — Fix B (typesCompatible argument check)", () => {
  ...
  test("ISOLATE-FixB: pass Cap<fs> as struct param — typesCompatible catches it", async () => {
    const r = await run(`struct MyS { x: Int }
    fn take(s: MyS) { s }
...
describe("Phase 10: Test isolation — Fix A (implMethods gate)", () => {
  ...
  test("ISOLATE-FixA: struct without impl for read() — implMethods gate catches it", async () => {
...
describe("Phase 10: Test isolation — explicit return type check", () => {
  ...
  test("ISOLATE-Return: explicit return of wrong type — return check catches it", async () => {
    const r = await run(`struct MyS { x: Int }
    impl MyS { fn read(self, p: String) -> String { "fake" } }
...
describe("Phase 10: Test isolation — implicit return type check (Phase 9)", () => {
  ...
  test("ISOLATE-ImplicitReturn: implicit return of wrong type — implicit return check catches it", async () => {
    const r = await run(`struct MyS { x: Int }
    impl MyS { fn read(self, p: String) -> String { "fake" } }
...
describe("Phase 10: Test isolation — struct field type check (Phase 8 Site 3)", () => {
  ...
  test("ISOLATE-StructField: field declared as MyS, constructed with Cap — field check catches it", async () => {
    const r = await run(`struct MyS { x: Int }
...
describe("Phase 10: Test isolation — typed let check (Phase 8 Site 4)", () => {
  ...
  test("ISOLATE-TypedLet: let x: MyS = env.fs — let type check catches it", async () => {
    const r = await run(`struct MyS { x: Int }
    fn main(env: Cap) {
        let x: MyS = env.fs
    }`);
...
describe("Phase 10: Test isolation — impl method argument check (Phase 8 Site 5)", () => {
  ...
  test("ISOLATE-ImplArg: impl method param declared as MyS, called with Cap — arg check catches it", async () => {
    const r = await run(`struct MyS { x: Int }
